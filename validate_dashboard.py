...
for g in data["groups"]:
    ...
    teacher_cost = round(rate * g["lessons_60min_per_month"], 2)
    total_cost = round(teacher_cost + g["cac_amortized_pm_usd"], 2)
    profit = round(g["total_rev_usd"] - total_cost, 2)
    margin = round((profit / g["total_rev_usd"]) * 100, 1) if g["total_rev_usd"] else 0.0
    breakeven = round(total_cost / g["rev_per_student_pm_usd"], 2) if g["rev_per_student_pm_usd"] else 0.0
